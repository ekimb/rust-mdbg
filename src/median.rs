...
pub fn get_column_consensus(bucket_seqs: &Vec<Vec<u64>>) -> Vec<u64> {
    let 
}

pub fn levenshtein(a: &Vec<u64>, b: &Vec<u64>) -> usize {
    let mut result = 0;
    ...
    /* Loop. */
    for (index_b, code_b) in b.iter() {
        result = index_b;
        distance_a = index_b;

        for (index_a, code_a) in a.iter() {
