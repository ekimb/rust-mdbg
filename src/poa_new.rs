...
#[derive(Default)]
pub struct POAGraph {
    graph: Graph<u32, i32, petgraph::Directed>,
    pub cns: Vec<u32>,
...
impl POAGraph {
    ...
    ///
    pub fn new_from_sequence(label: Option<Vec<u32>>, sequence: Option<Vec<u32>>) -> POAGraph {
    ...
    /// Create new POAGraph instance, optionally initialized with a sequence
    pub fn new() -> POAGraph {
        POAGraph::new_from_sequence(None::<Vec<u32>>, None::<Vec<u32>>)
    }
    ...
    ///
    pub fn add_edge(&mut self, in_node: NodeIndex, out_node: NodeIndex, weight: i32) -> EdgeIndex {
    ...
    pub fn consensus(&mut self) -> Vec<u32> {
        ...
        for node in self.consensus_path().to_vec() {
            self.cns.push(*self.graph.node_weight(node).unwrap() as u32);
        }
    ...
    ///
    pub fn align_sequence(&self, seq: Vec<u32>) -> Alignment {
        let score_fn = |a: u32, b: u32| if a == b { 1i32 } else { -1i32 };
        let mut aligner = pairwise::Aligner::new(-1, 0, &score_fn);
        aligner.semiglobal(seq, self.cns.to_vec())
    }
    ...
    ///
    pub fn incorporate_alignment(&mut self, aln: Alignment, label: &Vec<u32>, seq: Vec<u32>) {
        ...
        if aln.xstart > 0 {
            let (_start_node, end_node) = self.add_unmatched_sequence(&label, seq[..aln.xstart].to_vec());
            head_node = end_node;
        }
        if aln.xend < seq.len() {
            let (start_node, _end_node) = self.add_unmatched_sequence(&label, seq[aln.xend + 1..].to_vec());
            ...
            match op {
                ...
                AlignmentOperation::Subst => {
                    ...
                    let out_edge = self.graph
                        .edges(prev_node)
                        .find(|e| *self.graph.node_weight(e.target()).unwrap() as u32 == xchar)
                        .map(|e| e.id());
                    match out_edge {
                        Some(e) => {
                            *self.graph.edge_weight_mut(e).unwrap() += 1;
    ...
    pub fn topological_sort(&mut self) {
        ...
        let dfs = |graph: &Graph<u32, i32, petgraph::Directed>,
                   start: NodeIndex,
