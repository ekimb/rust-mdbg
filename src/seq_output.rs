...
pub fn write_minimizers_and_seq_of_kmers(output_prefix :&PathBuf, node_indices :&HashMap<Kmer,NodeIndex>, kmer_seqs :&HashMap<Kmer,String>, kmer_origin: &HashMap<Kmer,String>, dbg_nodes :&HashMap<Kmer,u32> , k:usize, l:usize, minim_shift: &HashMap<Kmer,(usize,usize)>)
{
    ...
    write!(file, "# [node name]\t[list of minimizers]\t[sequence of node]\t[abundance]\t[origin]\t[shift]\n").expect("error writing sequences file");
    ...
    for (node, index) in node_indices {
        ...
        let shift = &minim_shift[node];
        let s_line = format!("{}\t{}\t{}\t{}\t{}\t{:?}\n",index.index(),node.print_as_string(), seq, abundance, origin, shift);
