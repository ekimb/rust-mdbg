...
pub fn find_kmer_matches(seq1: &Vec<u32>, seq2: &Vec<u32>, k: usize) -> Vec<(u32, u32)> {
    if seq1.len() < seq2.len() {
        let set = hash_kmers((&seq1).to_vec(), k);
        return find_kmer_matches_seq1_hashed(&set, seq2.to_vec(), k);
    } else {
        let set = hash_kmers((&seq2).to_vec(), k);
        return find_kmer_matches_seq2_hashed(seq1.to_vec(), &set, k);
    }
}
...
pub fn hash_kmers<'a>(seq: Vec<u32>, k: usize) -> HashMapFx<Vec<u32>, Vec<u32>> {
    let slc : &Vec<u32> = seq.as_ref();
    let mut set: HashMapFx<Vec<u32>, Vec<u32>> = HashMapFx::default();
    for i in 0..(slc.len() + 1).saturating_sub(k) {
        set.entry(slc[i..i + k].to_vec())
...
pub fn find_kmer_matches_seq1_hashed(seq1_set: &HashMapFx<Vec<u32>, Vec<u32>>,
                                     seq2: Vec<u32>,
                                     k: usize)
                                     -> Vec<(u32, u32)> {
...
// already hashed by using the function sparse::hash_kmers
pub fn find_kmer_matches_seq2_hashed(seq1: Vec<u32>,
                                     seq2_set: &HashMapFx<Vec<u32>, Vec<u32>>,
                                     k: usize)
                                     -> Vec<(u32, u32)> {
...
pub fn expand_kmer_matches(seq1: Vec<u32>,
                           seq2: Vec<u32>,
                           k: usize,
